...
@Composable
fun Subscriptions_Screen(navController: NavHostController) {
    var expanded by remember { mutableStateOf(false) }
    var isEditing by remember { mutableStateOf(false) }
    var budgetLimit by remember { mutableStateOf("") }
    var savedLimit by remember { mutableStateOf("") }

    var netflixsub: String by remember { mutableStateOf("") }

    val netflix = netflixsub.toDoubleOrNull() ?: 0.0
    val totalSpent = netflix
    ...
    Scaffold (
        ...
        Column (
            ...
            Text("Track Subscriptions Spending", fontSize = 20.sp, fontWeight = FontWeight.Bold)
            ...
            OutlinedTextField(
                value = netflixsub,
                onValueChange = { netflixsub = it },
                shape = RoundedCornerShape(20.dp),
                label = {Text(text = "Netflix (KES)",
                    color = NewOrange,
                    fontSize = 20.sp,
                    fontFamily = FontFamily.Serif)},
                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number),
...
private fun SubscriptionsPreview() {
    Subscriptions_Screen(rememberNavController())
}