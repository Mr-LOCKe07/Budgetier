...
@Composable
fun Groceries_Screen(navController: NavHostController) {
    var expanded by remember { mutableStateOf(false) }
    var isEditing by remember { mutableStateOf(false) }
    var budgetLimit by remember { mutableStateOf("") }
    var savedLimit by remember { mutableStateOf("") }

    var groceriesInput: String by remember { mutableStateOf("") }

    val groceries = groceriesInput.toDoubleOrNull() ?: 0.0
    ...
    Scaffold (
        ...
        Column (
            ...
            Text("Track Groceries Spending", fontSize = 20.sp, fontWeight = FontWeight.Bold)
            ...
            Text("Total Spent: KES $groceries", fontSize = 18.sp, fontWeight = FontWeight.Bold)

            if (savedLimit.isNotEmpty()) {
                val limitValue = savedLimit.toDoubleOrNull()
                if (limitValue != null) {
                    val remaining = limitValue - groceries
                    Text(
...
@Preview
@Composable
private fun GroceriesPreview() {
    Groceries_Screen(rememberNavController())
}